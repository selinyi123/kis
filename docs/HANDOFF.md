...
- **KIS-016R（真实 GBrain 质量验证）状态：BLOCKED/PENDING**（2026-06-15）。本机无任何可调用 GBrain（PATH/pip/npm 均无；subprocess 优雅降级）。**执行 Agent 不伪造 GBrain 答案** → 不产出 A/B/C，KIS-016 keep_readonly 维持。已就绪：export96+baseline20+questions20+`manual-template`（20 题 stub）。拿到真实 GBrain 后走 subprocess 或 manual 一键评测（见 docs/KIS-016_ACCEPTANCE.md 的 KIS-016R 节）。新增 `manual-template` 子命令 + 测试（149/149 pytest）。**待用户：提供真实 GBrain（装 CLI 走 subprocess / 或别处跑完回填 manual JSONL）**。
