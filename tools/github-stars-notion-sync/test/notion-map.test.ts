...
test("maps an external starred repo", () => {
  ...
  assert.equal(properties["名称"], "openai/example");
  assert.equal(properties["Star 数"], 42);
  assert.equal(properties["归属"], "外部");
  assert.equal(properties["date:抓取日期:start"], "2026-08-30");
});
